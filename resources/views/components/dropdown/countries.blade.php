<div
	x-data="
		select({
			...
			country: @entangle('filters.country'),
...
<script>
	function select (config) {
		return {
			...
			init: function ()
			{
				this.options = JSON.parse(this.countries)

				if (!(this.selectedOptionIndex in this.options)) this.selectedOptionIndex = null
				
				this.$watch('search', ((value) => {
					if (!this.open || !value) return this.options = JSON.parse(this.countries)
			...
			selectOption: function ()
			{
				...
				this.$wire.set('filters.country', this.options[this.selectedOptionIndex].id)
			...
			toggleListboxVisibility: function ()
			{
				...
				this.focusedOptionIndex = this.options.findIndex(option => option.id === this.country)
			...
			{
				this.country = 0
				this.$wire.set('filters.country', 0)
